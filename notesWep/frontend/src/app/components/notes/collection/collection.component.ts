...
export class CollectionComponent {
  ...
  ngOnInit(): void {
    ...
    // Initialize note collection object
  ...
  getAllCollections(){
    this.noteService.get_all_collection(this.user._id).subscribe({
      next: (data) => {
        console.log(data)
  ...
  // Method to change note collection colour
  changeColour(color, note) {
    note.bgColor = color;
  }

  // Method to generate random note collection colour
  ...
  notesCollection(notes : ObjectID []) {
    return notes.length;
  }
    ...
    this.noteService.create_collection(this.note_collection).subscribe({
      ...
      error: (error) => {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'It was not possible to create the collection.',
        });
  ...
  // Method to delete note collection
  ...
  editCollection(note: Note_Collection) {
    console.log(`notes/collection/${note._id}`)
    this.router.navigate([`notes/collections/${note._id}`]);
  }

  // Method to navigate to friendships
  goToFriendships(note_collection: Note_Collection) {
    this.router.navigate([`notes/friendsListCollections/${note_collection._id}`]);
  }
}